...
html_4 <- c(
paste0('<p style="text-align: left; margin-top: +40px;"><small>Last updated <i>',
format(Sys.Date(), format="%B %d, %Y"),
'&ndash; Pulled automatically from my <a href="https://scholar.google.com/citations?hl=en&user=1W_3YjAAAAAJ">Google Scholar</a>.'), html_3)
# write the html list to a file
# Get the current date in the specified format
current_date <- format(Sys.Date(), "%Y-%m-%d")
# Create the file name with the current date
file_name <- paste0(current_date, "-publications.html")
writeLines(html_4, file_name)
